...
trait InputParser extends Op {
   ...
   */
  def inputstream: Try[java.io.InputStream] = if (filePath == null) Try(this.getClass().getClassLoader().getResourceAsStream("input.txt")) else
    Try(Files.newInputStream(Paths.get(filePath), StandardOpenOption.READ))
  ...
  def processFile(filePath: String) = {
    ...
    inputstream match {
      case Success(in) =>
        ...
        readLines(source).foreach(processLine _)
        source.close()
